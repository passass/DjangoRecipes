const recipe_blocks = $("#recipe_blocks")[0];
const recipe_order_by_categories = $(`#recipe_order_by_categories`)[0];
...
function imgSize(image_block) {
    let realWidth = image_block.naturalWidth;
    let realHeight = image_block.naturalHeight;
...
$.ajax({
    url: `/api/v1/recipe/?${searchParams.toString()}`,
    type: 'GET',
    success: (result) => {
        if (result["status"] == 200) {
...
const recipe_order_by = document.getElementById("recipe_order_by");
